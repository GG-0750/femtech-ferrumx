...
export default function Screen4Reminders({ onDone, patientId }) {
  ...
  useEffect(() => {
    ...
    const sub = supabase
      ...
      .on('postgres_changes', {
        ...
        if (payload.new.acknowledged) setAckStatus(true)
      })
      .subscribe()
  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
      ...
      {/* PHC Acknowledgement Status */}
      {patientId && (
        <div className="p-4 rounded-lg" style={{
          backgroundColor: ackStatus ? '#0A2420' : '#1A1D27',
          border: `1px solid ${ackStatus ? '#0F9B8E' : '#2A2D3A'}`
        }}>
          {ackStatus
            ? <p className="text-sm font-bold" style={{ color: '#0F9B8E' }}>✓ PHC has acknowledged this alert</p>
            : <p className="text-sm" style={{ color: '#8892A4' }}>⏳ Waiting for PHC acknowledgement...</p>
          }
        </div>
      )}
          ...
          {[7, 14, 28].map(days => (
            <div key={days} className="flex justify-between items-center p-3 rounded-lg"
              ...
              <div className="w-8 h-8 rounded-full flex items-center justify-center"
                style={{ backgroundColor: '#2A1A00', border: '1px solid #F5A623' }}>
      ...
      <button onClick={onDone}
        className="w-full py-5 rounded-lg font-bold text-base"
